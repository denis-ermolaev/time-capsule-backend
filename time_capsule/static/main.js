...
async function get_time(el, typeCapsule) {
  let url = "/oldversion/api/get_time";
...
async function get_post(num_page, typeCapsule, sorted_by, search) {
  ...
  let url = `/oldversion/api/get_private_capsule?${params}`;
  const response = await fetch(url, {
    method: "GET",
    headers: {
      "X-Requested-With": "XMLHttpRequest",
...
let init_capsules_list = async (num_page, typeCapsule, sorted_by, search) => {
  ...
  table_capsule_title.append(
    capsule_title,
    capsule_open_after,
    capsule_date_creation
  );
  ...
  result.capsules.forEach((element) => {
    ...
    a_capsule.href = `/oldversion/detail/${element[3]}`;
    a_capsule.innerHTML = `<div>${element[0]}</div><div>${element[1].slice(
      0,
      16
    )}</div><div>${element[2].slice(0, 16)}</div>`;
...
if (
  !document.querySelector("#details-detail") &&
  document.querySelector("#textarea-detail") &&
  deleteButtonOnly
) {
